...
@Autonomous(name = "observationZoneStates", group = "Iterative OpMode")
public class observationZoneStates extends OpMode {
    ...
    public void loop() {
        ...
        switch (stepW) {
            ...
            case 5: // back up to line up with submersible
                driveBackwards(pow);
                if(stepR == 1) {
                    if (pose[0] < -65) {
                        drive(0, 0, 0, 0);
            ...
            case 13: // return to origonal orientation
                turn();
                stepW (bufferOT, angleError);
                break;

            case 14: // strafe against wall
                strafeRight(pow);
                if(pose[1] > -5){
                    if (stepR == 1) {
                        stepW = 3;
